...
def test_prepare_clean_visual_source_uses_scaled_batch_rect(tmp_path, monkeypatch):
    ...
    task.visual_work_profile = {"fps": 25, "width": 1080, "height": 1920}
...
def test_burned_subtitle_ocr_reads_25fps_work_video_before_inpainting(tmp_path, monkeypatch):
    original = tmp_path / "original.mp4"
    original.write_bytes(b"video")
    work_video = tmp_path / "source-working-25fps.mp4"
    work_video.write_bytes(b"25fps-video")
    task = object.__new__(TransCreate)
    task.cfg = SimpleNamespace(
